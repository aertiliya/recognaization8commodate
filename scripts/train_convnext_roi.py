"""
第四步：训练 ConvNeXt V2 (防过拟合终极版)
包含：冻结/解冻迁移学习机制 + 强力 Dropout + 余弦退火
"""
...
def train_model(roi_dir='gt_roi_dataset', output_dir='convnext_models', epochs=20, batch_size=32, lr=0.0001, device='cuda'):
    ...
    # 增加 drop_rate 防止过拟合
    model = timm.create_model('convnextv2_atto', pretrained=True, num_classes=8, drop_rate=0.4).to(device)
    criterion = nn.CrossEntropyLoss()
    
    # ================= 核心策略：两阶段训练 =================
    
    # 阶段 1：冻结主干，仅训练分类头 (Epoch 1-5)
    print("\n❄️ 阶段 1: 冻结主干网络，仅训练分类头 (防止预训练权重被破坏)...")
    for name, param in model.named_parameters():
        if 'head' not in name:
            param.requires_grad = False
            
    # 只给分类头分配优化器 (学习率可以稍微大一点点)
    optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=lr * 5, weight_decay=0.05)
    
    best_acc = 0.0
    
    for epoch in range(1, epochs + 1):
        # 阶段 2：解冻主干，全量微调 (Epoch 6 以后)
        if epoch == 6:
            print("\n🔥 阶段 2: 解冻主干网络，开始全量微调...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=20) # 默认改为 20 轮
