...
def main():
    ...
    if 'A' in args.stages:
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source "{BASE}/image5/train" --output "{BASE}/yolo_dataset" --split train', BASE, "[A] 转换 Train")
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source "{BASE}/image5/val" --output "{BASE}/yolo_dataset" --split val', BASE, "[A] 转换 Val")
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source "{BASE}/image5/test" --output "{BASE}/yolo_dataset" --split test', BASE, "[A] 转换 Test")
