...
def main():
    ...
    if 'A' in args.stages:
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source image5/train --output "{BASE}/yolo_dataset" --train_ratio 1.0', BASE, "[A] 转换 Train 集")
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source image5/val --output "{BASE}/yolo_dataset" --train_ratio 1.0', BASE, "[A] 转换 Val 集")
        run_cmd(f'python "{scripts/"01_convert_json_to_yolo.py"}" --source image5/test --output "{BASE}/yolo_dataset/test" --train_ratio 1.0', BASE, "[A] 转换 Test 集")
